...
This compiles the client into a jar located at `./target/ThinkWhyMiddlewareApiSdk-1.0-SNAPSHOT.jar`. Note that this jar does not include any dependencies.
...
```perl
dependencies {
    compile 'thinkwhy.middleware.api.sdk:ThinkWhyMiddlewareApiSdk:1.0-SNAPSHOT'
}
```
...
<dependencies>
    <dependency>
        <groupId>thinkwhy.middleware.api.sdk</groupId>
        <artifactId>ThinkWhyMiddlewareApiSdk</artifactId>
...
This will compile the client and package all dependencies into a jar located at `./target/ThinkWhyMiddlewareApiSdk-1.0-SNAPSHOT.jar`.
...
```java
ThinkWhyMiddlewareApiSdkClientBuilder builder = ThinkWhyMiddlewareApiSdk.builder();
```
...
```java
ThinkWhyMiddlewareApiSdk client = ThinkWhyMiddlewareApiSdk.builder()
...
```java
ThinkWhyMiddlewareApiSdk client = ThinkWhyMiddlewareApiSdk.builder().build();
client.shutdown();
...
## Making requests
After a client is configured and created, you can make a request to the service. A method on the client interface (`ThinkWhyMiddlewareApiSdk`) is created for all actions (resource + method) in your API.
...
Service exceptions and client exceptions can be handled separately. Any exceptions modeled in the API will be a subtype of ThinkWhyMiddlewareApiSdkException.
```java
try {
    client.getApiRoot(...);
} catch(ThinkWhyMiddlewareApiSdkException e) {
   // All service exceptions will extend from ThinkWhyMiddlewareApiSdkException.
   // Any unknown or unmodeled service exceptions will be represented as a ThinkWhyMiddlewareApiSdkException.
